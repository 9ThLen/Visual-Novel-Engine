...
function addThemeContrastFindings(metadata: StoryMetadata | undefined, findings: StoryDoctorFinding[]): void {
  for (const issue of evaluateThemeContrast(metadata?.theme)) {
    findings.push({
      severity: 'warning',
      code: `theme.${issue.pair}Contrast`,
      messageKey: issue.backgroundDependent
        ? 'storyDoctor.issue.themeContrastBackgroundDependent'
        : 'storyDoctor.issue.themeContrast',
      messageParams: { pair: issue.pair, ratio: issue.ratio.toFixed(1) },
    });
  }
}
