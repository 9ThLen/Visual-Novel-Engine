...
8. buildShelves(stories: ShowcaseStory[], now: number): ShowcaseShelves:
   ...
   ПРАВИЛО МІНІМУМУ: тематична полиця з < 2 історіями повертається порожньою
   (її картки лишаються тільки в all). Бібліотека на 2-3 історії має показувати
   банер + «Всі історії», а не чотири заголовки з однією карткою під кожним —
   інакше вітрина виглядає порожнішою за звичайний список.
...
сортування, межа 14 діб, правило мінімуму: одна свіжа історія → fresh порожня,
але вона є в all); fallback-колір детермінований.
...
4. Скрім — вертикальний градієнт від прозорого до чорного на нижніх ~55%
   висоти, у ньому children. Суцільний блок ЗАБОРОНЕНО: він дає жорстку
   горизонтальну лінію посеред кадру. react-native-svg вже в залежностях —
   градієнт роби через <Svg><LinearGradient> (нових пакетів не додавати).
   Верхні ~45% кадру лишаються чистими.
...
3. Новий app/tabs/index.tsx:
   ...
   - Полиці: continueReading («Ти зупинився тут»), fresh («Нове»), quickReads
     ...
     ЛАНЦЮЖОК ЗОБРАЖЕННЯ постера (і банера): coverUri → uri з
     bannerBackgroundAssetId (фон стартової сцени) → лише тоді
     fallbackColorForSeed + літера. thumbnailUri в StoryMetadata опційний і в
     більшості історій порожній, тож без середньої ланки вітрина
     перетвориться на кольорові прямокутники.
     Мемоізовані картки.
...
1. Банер: LiveSceneBackdrop (~40% екрана); поверх — «Бібліотека» (назад) зліва
   вгорі. Кнопки «Студія» тут НЕ дублювати: єдині двері в авторський світ на
   цій сторінці — «Редагувати цю історію» в пункті 6. У скрімі: назва, рядок
   мети (автор · теги · readMinutes · 'кінцівок: N'), рейтинг, CTA
   «Читати»/«Продовжити».
   Рейтинг у шапці — тільки коли aggregate.verdict !== null (тобто count >= 3):
   зірки + 'X.X · N рецензій · вердикт'. Інакше рядка рейтингу в шапці НЕМАЄ
   (порожній агрегат у шапці читається як «історія погана»).
   Під CTA дрібно 'зі сцени: <назва>', якщо є lastSceneId. Назву бери зі
   SceneRecord.name по lastSceneId (канонічне поле з ProjectScene; 'sceneName'
   у редакторі — легасі) — НЕ з saveSlot.sceneName: у
   createSaveSlot (lib/story-domain.ts) туди кладеться id сцени, і в UI
   з'явиться щось на кшталт 'scene-7f3a'. Немає назви → рядок не рендериться.
...
5. Рецензії — ДВА РЕЖИМИ, залежно від count:
   а) count < 3 (нормальний стан Етапу 1 — рецензії лише з цього пристрою):
      блок називається «Твоя оцінка». БЕЗ агрегату, БЕЗ гістограми, БЕЗ
      перемикача сортування, БЕЗ 'Корисно' — велика цифра «5.0 · 1 рецензія»
      і гістограма з однією смужкою виглядають як зламаний Steam. Є своя
      рецензія → показати її картку з «Змінити». Немає → «Оціни цю історію»
      + кнопка «Оцінити». Чужі рецензії (якщо раптом є) — простим списком
      карток без голосування.
   б) count >= 3: повний Steam-режим — перемикач helpful/recent, агрегат
      (велика цифра + розподіл смужками 5→1), картки (коло-ініціали
      authorName, ім'я, бейдж 'дочитав · N кінцівок', зірки, текст,
      'Корисно · N' → toggleHelpful).
...
3. УВАГА: фінального екрана НЕМАЄ. finishStory (app/reader.tsx) одразу робить
   router.replace('/tabs') — читача викидає в бібліотеку. Тому оверлей не
   «накладається на фінал», а ЗАТРИМУЄ вихід: finishStory показує
   PostFinaleRating замість негайного replace, а replace('/tabs') виконується
   в обробнику «Зберегти»/«Пізніше»/закриття. Якщо shouldPromptForReview дає
   false — поведінка стара, replace одразу. stopReaderPlayback лишається там,
   де зараз (звук глушиться при фіналі, не при виході).
   components/reader/PostFinaleRating.tsx — компактний оверлей:
...
5. Сторінка цієї новели показує блок «Твоя оцінка» з твоєю рецензією (без
   агрегату й гістограми — рецензія одна); «Змінити» працює; повторний прохід
   тієї ж кінцівки оверлей не показує.
