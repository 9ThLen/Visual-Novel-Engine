...
**Статус:** реалізовано 2026-08-26 (R4.0–R4.4). Створено 2026-08-26.
...
Відтворення в сітці не означає програвач у кожній плитці. Екран тримає **один** контролер прев'ю (`useAudioPreview`), плитка лише запитує «зіграй мене» і малює стан за порівнянням `playingKey === item.key`. Запуск другого звуку зупиняє перший. Інспектор користується тим самим контролером — інакше та сама доріжка грала б двічі.
...
- по центру — кнопка **відтворити / зупинити** (`play` ↔ `stop`), вона ж головна афорданс-точка плитки;
