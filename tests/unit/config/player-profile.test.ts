...
import metroBlockList from '../../../config/metro-blocklist';
import playerProfile from '../../../config/player-profile';
...
describe('the player metro blockList', () => {
  it('blocks the authoring trees on both path separators', () => {
    expect(blocks(POSIX_ROOT, `${POSIX_ROOT}/src/components/editor/PlayMode.tsx`)).toBe(true);
    expect(blocks(WINDOWS_ROOT, `${WINDOWS_ROOT}\\src\\components\\editor\\PlayMode.tsx`)).toBe(true);
    expect(blocks(POSIX_ROOT, `${POSIX_ROOT}/src/lib/vn-plate-editor/anything.ts`)).toBe(true);
  });

  it('leaves the reader alone', () => {
    expect(blocks(POSIX_ROOT, `${POSIX_ROOT}/src/components/reader/ReaderDisplay.tsx`)).toBe(false);
    expect(blocks(WINDOWS_ROOT, `${WINDOWS_ROOT}\\src\\components\\reader\\ReaderDisplay.tsx`)).toBe(false);
  });

  /**
   * `src/lib/document-editor/scene-graph-*` is graph traversal the reader's coverage
   * code walks, and `src/lib/ai/permissions.ts` is read by `src/lib/user-settings.ts`.
  ...
  it('does not block shared code that merely sits in an authoring directory', () => {
    expect(blocks(POSIX_ROOT, `${POSIX_ROOT}/src/lib/document-editor/scene-graph-traversal.ts`)).toBe(false);
    expect(blocks(POSIX_ROOT, `${POSIX_ROOT}/src/lib/ai/permissions.ts`)).toBe(false);
  });

  it('does not block a dependency whose path happens to contain a blocked name', () => {
    expect(
      blocks(POSIX_ROOT, `${POSIX_ROOT}/node_modules/some-package/src/components/editor/index.js`),
    ).toBe(false);
  });
});
