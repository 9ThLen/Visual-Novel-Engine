...
describe('source upload pruning', () => {
  it('keeps only Android-reachable source files', () => {
    ...
    write(root, 'src/components/Reader.tsx', 'keep');
    write(root, 'src/components/Editor.tsx', 'remove');
    write(root, 'src/lib/helper.android.ts', 'keep');
    write(root, 'src/lib/helper.web.ts', 'remove');
    ...
    const removed = pruneUnreachableSource(root, new Set([
      ...
      'src/lib/helper.android.ts',
    ]));

    expect(removed.sort()).toEqual(['src/components/Editor.tsx', 'src/lib/helper.web.ts']);
    expect(fs.existsSync(path.join(root, 'src', 'components', 'Reader.tsx'))).toBe(true);
...
`);
  ...
  it('catches authoring code that came along', () => {
    root = stagedProject();
    write(root, 'src/components/document-editor/Whatever.tsx', 'export default null;');
    expect(verifyStagedAndroidProject(root).join('\n')).toContain('src/components/document-editor is still');
  });
...
describe('the staging API EAS ownership invariant', () => {
  it('refuses a missing project id unless engine use is explicit', async () => {
    ...
    await expect(stageAndroidProject({
      releaseFile: path.join(REPO_ROOT, 'tests', 'e2e', 'player', '.demo.vnerelease'),
      outDir,
      repoRoot: REPO_ROOT,
...
describe('modules the player build substitutes', () => {
  ...
  it('has some, and they are the ones the profile names', () => {
    expect(substituted).toContain('src/stores/use-app-store.ts');
  ...
  it('survive the prune that removes everything unreachable', () => {
    const root = tempDir();
    try {
      for (const file of [...substituted, 'src/lib/orphan.ts', 'src/lib/kept.ts']) {
        write(root, file, 'export const x = 1;');
      }
      const removed = pruneUnreachableSource(root, new Set(['src/lib/kept.ts']));
      ...
      expect(removed).toContain('src/lib/orphan.ts');
      expect(fs.existsSync(path.join(root, 'src', 'lib', 'kept.ts'))).toBe(true);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  /**
   * `src/stores/use-app-store.ts` is on both lists: forbidden from the player's
  ...
  it('are reported when they are missing, naming what the bundler would say', () => {
    ...
    try {
      ...
      expect(problems).toContain('src/stores/use-app-store.ts');
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
