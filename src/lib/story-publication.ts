 ...
 *
 * Lives beside `src/lib/story-theme.ts` rather than inside `src/lib/story-domain.ts` or
 * `src/lib/release/` for one structural reason: `story-domain` describes the stored
 * story and `src/lib/release` describes the published artifact, and both need these
