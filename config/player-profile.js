...
const PLAYER_STORE_SUBSTITUTION = {
  from: 'src/stores/use-app-store.ts',
  to: 'src/stores/use-app-store.player.ts',
};
 ...
 * `src/stores/use-app-store.ts` as unreachable — the graph walk applies these
...
const PLAYER_MODULE_SUBSTITUTIONS = [
  ...
  { from: 'src/lib/bundled-assets.ts', to: 'src/lib/bundled-assets.player.ts' },
];
 ...
 * — `src/lib/document-editor/scene-graph-*`, which the reader's coverage code walks,
 * and `src/lib/ai/permissions.ts`, which `src/lib/user-settings.ts` reads — are handled
...
const PLAYER_BLOCKED_TREES = [
  ...
  'src/components/story-home',
  'src/components/vn-plate-editor',
  'src/components/media-library',
  'src/lib/editor',
  'src/lib/vn-plate-editor',
  'src/lib/story-home',
  'src/lib/scene-document',
];
...
const PLAYER_FORBIDDEN_MODULES = [
  ...
  'src/stores/app-store-slices/story-slice.ts',
  'src/stores/app-store-slices/snapshots-slice.ts',
  'src/stores/app-store-slices/scene-write-slice.ts',
  'src/stores/app-store-slices/scene-slice.ts',
  'src/stores/app-store-slices/libraries-slice.ts',
  'src/lib/story-snapshots.ts',
  'src/lib/ai/change-set.ts',
  'src/lib/release/compile.ts',
  'src/lib/release/service.ts',
  'src/lib/release/shell-build.ts',
  'src/lib/release/asset-sources.ts',
  'src/lib/release/bundle-file.ts',
  // The build client talks to a local helper that holds signing credentials.
  // Nothing a published player does involves it.
  'src/lib/release/build-client.ts',
  'src/lib/story-backup/capture.ts',
  'src/lib/story-backup/import.ts',
  'src/lib/story-backup/service.ts',
];
