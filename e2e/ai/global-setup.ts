 ...
 */
/**
 * When the app never paints, the useful evidence is the bundle response the page
 * asked for — Metro answers a failed build with a 500 and a JSON payload naming
 * the module it could not handle. Surface it instead of leaving a blank screen.
 */
async function reportBundleFailure(page: Page, appOrigin: string): Promise<void> {
  try {
    const bundleUrl = await page.evaluate(() =>
      Array.from(document.querySelectorAll('script[src]'))
        .map((script) => (script as HTMLScriptElement).src)
        .find((src) => src.includes('.bundle')) ?? null);
    console.warn(`[global-setup] bundle url: ${bundleUrl ?? 'not found in page'}`);
    const response = await page.request.get(bundleUrl ?? appOrigin);
    console.warn(`[global-setup] bundle status: ${response.status()} ${response.headers()['content-type'] ?? ''}`);
    if (!response.ok()) console.warn(`[global-setup] bundle body: ${(await response.text()).slice(0, 4000)}`);
  } catch (error) {
    console.warn(`[global-setup] could not read the bundle response: ${String(error)}`);
  }
}
...
async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    await reportBundleFailure(page, appOrigin);
  } finally {
    await browser.close();
  }
}
