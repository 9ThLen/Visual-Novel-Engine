...
- **Гейт гідратації.** Сцени приїжджають асинхронно, і до того кожен файл виглядає невикористаним. Деструктивні дії (відчеплення й прибирання файлу) не пропонуються, доки `hydrateSceneRecordsForStory` не завершився **і** пам'ять не покриває `story.sceneCount` — завершене завантаження, яке нічого не повернуло, від історії без сцен відрізняє саме лічильник.
- **Повторна перевірка на записі.** Гейт закриває широке вікно, але не вузьке — стор може змінитися між рендером кнопки і натисканням. Тому `handleDetachFromCharacter` читає `useAppStore.getState()`, перебудовує галерею (`findOwnerInGallery`) і ще раз питає `canDetachOwner`; відчеплення виконується над **свіжим** списком персонажів, інакше зовнішній запис (збереження редактора, AI-відкат) був би затертий цією дією.
...
**Мутаційно перевірено шість вузлів:** ремонт `defaultSpriteId`, per-owner `usage`, save-barrier на вході, гейт гідратації, свіже читання стора на записі, відмова `canDetachOwner` у вузькому вікні.
