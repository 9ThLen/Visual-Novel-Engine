...
### R2 — Адитивна власність + інтеграція

- «Додати персонажу…» створює новий `CharacterSprite` з новим id і тим самим URI; жодна сцена не змінюється.
- «Прибрати з ‹персонаж›» за `enabled + disabled === 0`, з синхронним fallback і для `defaultSpriteId`, і для `authoring.currentSpriteId` — редактор уже робить це при видаленні спрайта ([embedded-script.ts:5410](lib/vn-plate-editor/embedded-script.ts:5410)), два шляхи не повинні розійтися.
- Source-aware видалення в інспекторі: «Прибрати імпортований файл» / «Прибрати з ‹персонаж›».
- Передавати `sceneId` у params; «Відкрити у поточній сцені»; прибрати фолбек на `scenes[0]?.id` ([story-gallery.tsx:52](app/story-gallery.tsx:52)).
