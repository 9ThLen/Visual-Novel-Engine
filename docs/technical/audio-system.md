...
│  src/lib/audio-manager-enhanced.ts (187 LOC) │
├──────────────┬───────────────┬───────────┤
...
### AudioPlayerService
- **File:** `src/lib/audio-player-service.ts`
...
### AudioLibraryService
- **File:** `src/lib/audio-library-service.ts`
...
- **File:** `src/lib/audio-trigger-scheduler.ts`
...
### EnhancedAudioManager (Facade)
- **File:** `src/lib/audio-manager-enhanced.ts`
...
`src/lib/reader-audio-session.ts` manages whether reader audio is active:
...
| File | LOC | Purpose |
|---|---|---|
| `src/lib/audio-interfaces.ts` | 93 | TypeScript interfaces |
| `src/lib/audio-types.ts` | 88 | Type definitions |
| `src/lib/audio-manager-enhanced.ts` | 187 | Main facade (singleton) |
| `src/lib/audio-player-service.ts` | ~100 | Playback service |
| `src/lib/audio-library-service.ts` | ~91 | Library management |
| `src/lib/audio-trigger-scheduler.ts` | ~80 | Trigger orchestration |
| `src/lib/reader-audio-session.ts` | ~20 | Session guard |
| `src/lib/audio-web-source.ts` | ~25 | Web audio source helper |
| `src/lib/audio-library.ts` | ~20 | Legacy audio library |
