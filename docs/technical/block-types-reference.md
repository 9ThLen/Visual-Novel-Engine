...
Source: `src/lib/engine/types.ts` (lines 16-111)
Factories: `src/lib/engine/event-factory.ts`
...
- **Lightning/thunder:** flashes are driven by the shared scheduler in `src/lib/engine/lightning-scheduler.ts`; thunder claps follow each strike when `sound` is enabled.
- **Auto-executes:** Yes
...
Source: `src/lib/engine/conditionUtils.ts`
