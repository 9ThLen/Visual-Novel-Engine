...
- Types: `src/stores/app-store-types.ts`.
- Initial state: `src/stores/app-store-initial-state.ts`.
- Slice implementations: `src/stores/app-store-slices/*`.
...
`src/lib/app-store-persistence.ts` owns Zustand `partialize`, `merge`, and version migration.
`src/lib/app-store-storage.ts` wraps storage and extracts large scene maps into sidecar scene storage.
`src/lib/scene-record-storage.ts` owns per-story and per-scene scene record storage.
...
Selector helpers in `src/stores/use-app-store.ts` delegate scene lookup to `src/lib/scene-access.ts`.

Reader paths use `src/lib/reader-scene-cache.ts` to hydrate the current scene plus bounded prefetch scenes. Editor/home paths use full-story hydration when they need the complete scene map.
