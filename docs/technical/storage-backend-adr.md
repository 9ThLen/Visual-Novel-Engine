...
- **Active data loss today:** `getPersistableMediaLibrary` ([src/lib/app-store-persistence.ts](../../src/lib/app-store-persistence.ts)) drops any `data:` URI `> 256 KB`/file or `> 1 MB` total on **every** persist. Real images already vanish on web reload.
...
- No hosted backend exists. `src/constants/oauth.ts` is a **Manus template** (empty env, expects a separate API on :3000) — not reusable for Supabase Auth. `tools/ai-bridge` is localhost-only (`127.0.0.1`).
...
- New `src/lib/idb-storage.ts`: one DB `vne-storage` with object stores `kv` and `media`. The module has no browser API work at import time, so the shared import remains native-safe. It exports an async `StorageLike` (`getItem/setItem/removeItem`) over `kv`; Blob helpers are added in Phase 2 when first needed.
- [src/lib/persistent-storage.ts](../../src/lib/persistent-storage.ts): in the `Platform.OS === 'web'` branch, return the IDB `StorageLike` instead of `createSafeWebStorage(localStorage)`. Keep the JSON-validation guard for `APP_STATE` and the try/catch + toast semantics.
...
- No change to `createAppStoreStorage` ([src/lib/app-store-storage.ts](../../src/lib/app-store-storage.ts)) or the store wiring (`createJSONStorage(createAppStoreStorage)`, [src/stores/use-app-store.ts:216](../../src/stores/use-app-store.ts)) — already async-compatible.
- Also covers the direct write at [use-app-store.ts:238](../../src/stores/use-app-store.ts).
...
- **Single choke point** — [src/lib/media-library-service.ts](../../src/lib/media-library-service.ts) `addAssetToLibraryPure`, invoked via [src/stores/media-library-actions.ts](../../src/stores/media-library-actions.ts) `addAssetToLibrary`: on web, when the incoming `uri` is a `data:`/`blob:`, decode to a `Blob`, persist it under the selected storage key (skip when already present), and set `asset.uri = idb://media/<storage-key>`.
- **Cover every web upload entry point** so none bypasses the choke point:
  - [src/lib/pick-image.ts](../../src/lib/pick-image.ts), [src/lib/web-file-input.ts](../../src/lib/web-file-input.ts) (file pick / drag-drop)
  - [src/lib/remove-background.web.ts](../../src/lib/remove-background.web.ts) (background-removal output)
  - editor paste — [src/components/vn-plate-editor/PlateWebViewEditor.web.tsx](../../src/components/vn-plate-editor/PlateWebViewEditor.web.tsx)
  - audio path — [src/lib/audio-web-source.ts](../../src/lib/audio-web-source.ts)
...
- **Resolution:** [src/lib/asset-resolver.ts](../../src/lib/asset-resolver.ts) + [src/lib/media-library-service.ts](../../src/lib/media-library-service.ts) `resolveLibraryAssetUri` learn `idb://media/<storage-key>` → `getBlob` → `URL.createObjectURL`, with a bounded in-memory `Map<key, objectURL>` cache. Object URLs are revoked on eviction and cache clear.
- Direct React Native `<Image>` consumers that display persisted library assets use [src/components/resolved-asset-image.tsx](../../src/components/resolved-asset-image.tsx), so stable IDB references are resolved before rendering.
...
- One-time migration: [src/lib/web-media-migration.ts](../../src/lib/web-media-migration.ts) moves each persisted `LibraryAsset` and historical character sprite still holding a `data:` URI through the Phase-2 Blob write, preserving asset/sprite IDs while rewriting the URI to `idb://media/<storage-key>`.
...
- Once enabled, `getPersistableMediaLibrary` ([src/lib/app-store-persistence.ts](../../src/lib/app-store-persistence.ts)) rejects any `data:` **or ephemeral `blob:`** URI with a development warning instead of silently applying size-based eviction.
- **The migration must not be bound to one screen.** It runs in [src/stores/storage-bootstrap.ts](../../src/stores/storage-bootstrap.ts), an idempotent shared promise kicked off by the root layout ([app/_layout.tsx](../../app/_layout.tsx)) and awaited by Home. `document-editor`, `reader`, `story-home` etc. are top-level routes, so a web page refresh lands on them directly; gating the migration on Home would leave those sessions unmigrated with the lossy caps still active.
...
Exercise: import/export (still 10 MB JSON, media by reference — [src/lib/story-hooks.ts:38](../../src/lib/story-hooks.ts)), reader window + prefetch, document editor, audio playback, background removal round-trip, hard reload, and native build (must be untouched — file paths, not `idb://`).
...
- Shipped as [src/lib/web-media-cleanup.ts](../../src/lib/web-media-cleanup.ts), run from [src/hooks/useLibraryBootstrap.ts](../../src/hooks/useLibraryBootstrap.ts) with a seven-day grace period (`WEB_MEDIA_GC_GRACE_MS`): a Blob has to be unreferenced across two separate runs a week apart before it is removed.
...
- **`BackupManifest` (Phase 6 implemented)** — versioned, decoupled from the Zustand envelope: `{ schemaVersion, backupId, createdAt, appVersion, stories, scenes, libraries, assets:[{ assetId, storageKey, sha256, mimeType, size }] }`. [src/lib/backup-service.ts](../../src/lib/backup-service.ts) defines the transport-independent orchestration.
- **Phase 7 implemented:** [src/stores/backup-local-repository.ts](../../src/stores/backup-local-repository.ts) captures canonical data (scenes from scene storage, see Invariants), stages IDB/native assets, creates pre-restore story snapshots, and activates restored state with rollback to the *complete* prior scene set; [src/lib/backup-crypto.ts](../../src/lib/backup-crypto.ts) supplies SHA-256 over `BackupBinary`; [src/lib/supabase-backup.ts](../../src/lib/supabase-backup.ts) supplies persisted Supabase Auth, immutable uploads (resumable > 6 MB on web, single-shot on native — TUS needs a Blob), private downloads, and `listBackups()`. Private-bucket/table RLS is versioned in [supabase/migrations/20260714000000_backup_storage.sql](../../supabase/migrations/20260714000000_backup_storage.sql).
- **Phase 7 UI:** [app/cloud-backup.tsx](../../app/cloud-backup.tsx) (sign in, back up, list, restore, delete), reached from the Cloud section of [app/settings.tsx](../../app/settings.tsx); state in [src/hooks/use-cloud-backup.ts](../../src/hooks/use-cloud-backup.ts); the client/service is built once in [src/stores/cloud-backup.ts](../../src/stores/cloud-backup.ts).
...
- **Sync (8):** `stories`/`scenes` in Postgres, `SceneRecord` as JSONB + `revision`; outbox + revision-check first. Reuses the `STALE_REVISION` concept already in [src/lib/bridge-protocol.ts](../../src/lib/bridge-protocol.ts).
