...
files_reviewed:
  ...
  - src/components/document-editor/document-command-ui.ts
  ...
  - tests/unit/lib/story-doctor.test.ts
  - tests/unit/lib/useSceneExecutor-labels-goto.test.ts
finding_counts:
  critical: 0
  warning: 3
  info: 2
...
- Evidence: `src/lib/scene-document/sceneSerializer.ts:55-62` emits `[label ...]` and `[goto ...]` syntax.
- Evidence: `src/lib/scene-document/sceneParser.ts:120-142` recognizes only `background`, audio, and `choice`; every other bracket command becomes a generic `command` node.
- Evidence: `src/lib/scene-document/sceneValidation.ts:30-36` consequently reports these serialized commands as unknown rather than validating them as label/goto nodes.
...
- Evidence: `src/lib/scene-document/sceneTypes.ts:127-130` and `src/lib/engine/types.ts:294-299` allow `contains`, `isEmpty`, `has`, and `not_has` in addition to comparison operators.
- Evidence: `src/lib/vn-plate-editor/embedded-script.ts:1817` limits the popover to `==`, `!=`, `>`, `<`, `>=`, and `<=`.
- Evidence: `src/lib/vn-plate-editor/embedded-script.ts:2037-2039` attempts to select the stored operator from that restricted list, while `src/lib/vn-plate-editor/embedded-script.ts:2093-2095` writes the select's current value back on save.
...
- Evidence: `src/lib/story-doctor.ts:382` trims label names and `src/lib/story-doctor.ts:421` trims goto targets before comparing them.
- Evidence: `src/lib/engine/useSceneExecutor.ts:39-46` compares the raw stored label name to the raw target with strict equality and no trimming.
